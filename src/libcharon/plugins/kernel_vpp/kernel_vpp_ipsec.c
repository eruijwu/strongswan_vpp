...
static status_t manage_bypass(bool add, uint32_t spd_id)
{
    ...
    ep->is_outbound = 0;
    ep->policy = htonl(IPSEC_API_SPD_ACTION_BYPASS); // 0
    // ep->is_ip_any = 1;
    // ip_any is not support by VPP ??
    ep->local_address_stop.af = htonl(ADDRESS_IP6);
    ep->remote_address_stop.af = htonl(ADDRESS_IP6);
    memset(ep->local_address_stop.un.ip6, 0xFF, 16);
    memset(ep->remote_address_stop.un.ip6, 0xFF, 16);
                              ...
                              kernel_ipsec_manage_policy_t *data)
{
    ...
    {
        case POLICY_IPSEC:
            ep->policy = IPSEC_API_SPD_ACTION_PROTECT;
            break;
        case POLICY_PASS:
            ep->policy = IPSEC_API_SPD_ACTION_BYPASS;
            break;
        case POLICY_DROP:
            ep->policy = IPSEC_API_SPD_ACTION_DISCARD;
            break;
    }
    ...
    {
	ep->local_address_start.af = htonl(is_ip6 ? ADDRESS_IP6: ADDRESS_IP4);
	ep->local_address_stop.af = htonl(is_ip6 ? ADDRESS_IP6: ADDRESS_IP4);
	ep->remote_address_start.af = htonl(is_ip6 ? ADDRESS_IP6: ADDRESS_IP4);
	ep->remote_address_stop.af = htonl(is_ip6 ? ADDRESS_IP6: ADDRESS_IP4);
        memcpy(ep->local_address_start.un.ip6, src_from.ptr, src_from.len);
        memcpy(ep->local_address_stop.un.ip6, src_to.ptr, src_to.len);
        memcpy(ep->remote_address_start.un.ip6, dst_from.ptr, dst_from.len);
        memcpy(ep->remote_address_stop.un.ip6, dst_to.ptr, dst_to.len);
    }
    ...
    kernel_ipsec_add_sa_t *data)
{
    ...
    ep->sad_id = htonl(sad_id);
    ep->spi = id->spi; //attention: don't htonl
    ep->protocol = (id->proto == IPPROTO_ESP ? htonl(IPSEC_API_PROTO_ESP): htonl(IPSEC_API_PROTO_AH));
    switch (data->enc_alg)
    {
        case ENCR_NULL:
    ...
    ep->integrity_algorithm = htonl(ia);
    ...
    if (data->esn)
    {
	ep->flags |= IPSEC_API_SAD_FLAG_USE_EXTENDED_SEQ_NUM;
    }

    is_ip6 = (AF_INET6 == id->src->get_family(id->src));
    if (data->mode == MODE_TUNNEL)
    {
        ep->flags |= IPSEC_API_SAD_FLAG_IS_TUNNEL;
        ep->flags |= is_ip6 ? IPSEC_API_SAD_FLAG_IS_TUNNEL_V6 : 0;
    }

    ep->flags = htonl(ep->flags);

    ep->tunnel_src.af = is_ip6 ? ADDRESS_IP6 : ADDRESS_IP4;
    src = id->src->get_address(id->src);
    memcpy(ep->tunnel_src.un.ip6, src.ptr, src.len);
    ep->tunnel_dst.af = is_ip6 ? ADDRESS_IP6 : ADDRESS_IP4;
    dst = id->dst->get_address(id->dst);
    memcpy(ep->tunnel_dst.un.ip6, dst.ptr, dst.len);
    if (vac->send(vac, (char *)mp, sizeof(*mp), &out, &out_len))
    {
